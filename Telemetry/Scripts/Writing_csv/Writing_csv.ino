...
void loop() // run over and over again
{
  ...
  if (millis() - timer > 2000) {
    ...
    if (GPS.fix) {
      myFile = SD.open("test_2", FILE_WRITE);
      
      if(LastLat != (String)GPS.latitude || LastLong != (String)GPS.longitude){
        Serial.println(DDMMToDD((String)GPS.latitude, 6) + ",-" + DDMMToDD((String)GPS.longitude , 6));
        myFile.println(DDMMToDD((String)GPS.latitude, 6) + ",-" + DDMMToDD((String)GPS.longitude , 6));
